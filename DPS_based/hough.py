# 基于哈夫变换的文档矫正。从github上找的，未测试
# ref: https://github.com/JianghaoLan/doc-rectify-opencv
...
def remove_inner_lines(lines, max_dalpha):
    ...
    dalpha = line_theta_diff(
        np.dot(alpha2D_row.T, np.ones_like(alpha2D_row)),
...
def show_lines(
    lines, im, rgb=(255, 0, 0), text=None, line_thickness=2, show_immediately=True
):
...
def connect_lines(lines, thresh1=np.pi / 90, thresh2=np.pi / 125):
    ...
    for i in range(len(lines)):
        ...
        if np.any(parallel):
            ...
            theta_attempt1 = calc_theta(
                x1_i, y1_i, x1[i + 1 :].flatten(), y1[i + 1 :].flatten()
            ).flatten()[pal_ind]
            ...
            theta_attempt2 = calc_theta(
                x1_i, y1_i, x2[i + 1 :].flatten(), y2[i + 1 :].flatten()
            ).flatten()[pal_ind]
...
def calc_vector_angle(v1, v2):
    # 计算向量夹角
    return np.arccos(
        np.inner(v1, v2) / (np.linalg.norm(v1, axis=-1) * np.linalg.norm(v2, axis=-1))
    )
...
def calc_pts_img(lines, start_vector=np.array([-1, 0])):
    ...
    ABC22 = ABC2[:, [1]]
    x, y = calc_node(
        np.hstack([ABC11, ABC11, ABC12, ABC12]), np.hstack([ABC21, ABC22, ABC22, ABC21])
    )
...
if __name__ == "__main__":
    ...
    warped = rectify_doc(
        img, w_h=img_wh, start_vector=start_vector, show_every_step=True
    )
